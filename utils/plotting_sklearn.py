...
def plot_roc_curve(probas, y_test, title="ROC Curve", x_lab="False Positive Rate (FPR)", 
                  ...
                  col_map="viridis", savefigure=False, filename=None, size=(10,10), fontsize=20,
...
def precision_recall_data(y_test, probs, pos_label=1):
    """
    Function to get the precision recall data
    """
    log = logging.getLogger(__name__)
...
def plot_confusion_matrix(cmx, axes=None, col_map="RdYlBu", labels=(0, 1), title="Confusion Matrix", x_label="Predicted Class", y_label="Known Class", 
                          fontsize=20, annotate=False, vmin=None, vmax=None, font_scale=1.4):
    """
    Function to plot a heatmap of a confusion matrix
    """
    log = logging.getLogger(__name__)
    ...
    axes = sns.heatmap(cmx_df, annot=annotate, ax=axes, fmt="d", cmap=col_map, vmin=vmin, vmax=vmax)
...
def plot_metrics(df, predicted_column_name="prediction", known_column_name="known", probabilities=None, 
    ...
    if all_classes is True:
        ...
        cmx = cmet.get_multi_label_confusion_matrix(df, predicted_column_name=predicted_column_name, known_column_name=known_column_name, labels=labels, return_dict=False)
        for ith in range(len(classes)):
            axs[ith] = plot_confusion_matrix(cmx[ith], axes=axs[ith], col_map=col_map, title="Confusion Matrix Class {}".format(int(lclasses[ith])), 
                                             annotate=annotate, fontsize=fontsize, vmin=vmin, vmax=vmax, font_scale=font_scale)
       
        if roc_curve is True:
            log.info(len(classes))
            axs[len(classes)] = plot_roc_curve(probabilities, df[known_column_name].values, axes=axs[len(classes)] , plot_class="all", col_map=col_map, fontsize=fontsize)
        
        if pr_curve is True:
            axs[-1] = plot_pr_curve(probabilities, df[known_column_name].values, axes=axs[-1], plot_class="all", col_map=col_map, fontsize=fontsize)
        ...
        cmx = cmet.get_confusion_matrix(df, predicted_column_name=predicted_column_name, known_column_name=known_column_name, labels=labels, return_dict=False)
        
        axs[0] = plot_confusion_matrix(cmx, axes=axs[0], col_map=col_map, fontsize=fontsize, annotate=annotate, vmin=vmin, vmax=vmax)
       
        if roc_curve is True:
            log.info(len(classes))
            axs[1] = plot_roc_curve(probabilities, df[known_column_name].values, axes=axs[1] , plot_class="all", col_map=col_map, fontsize=fontsize)
